...
# Se registra el cambio en la pila de historial.(Historial de cambios)
    ...
    #variable para ala¿macenar la nota
    nota_ingresada = input("Ingrese la nota obtenida en ese curso:\n ").strip()
     
     # Verificar si la nota está vacía
    if not nota_ingresada:
        print(" Cada curso debe tener una nota asignada.\n")
        return
...
#POST : Se imprime en pantalla la lista completa de cursos y notas.
    ...
    if not cursos:#No hay cursos
       ...
       for i in range(len(cursos)): 
           #mostrarlo en una lista, numerada
...
#POST: Se imprime la cantidad de cursos aprobados y reprobados.
   ...
   #Variables nuevas para almacenar los reprobados y aprobados
    ...
    for nota in notas: 
        if nota >= 60:# rango para aprobados
            aprobados += 1
        else:
            reprobados += 1
...
def Eliminar_Curso():
    ...
    cursos_lower = [c.lower() for c in cursos]#cursos en minusculas
    ...
    if Eliminar.lower() in cursos_lower:
        ...
        confirmacion = input(f"¿Está seguro que desea eliminar '{curso_eliminado}'? (si/no): ").strip().lower()
        if confirmacion == "si":
            cursos.pop(indice)
            notas.pop(indice)
...
#POST:Las listas se reordenan y se muestran de mayor a menor nota, en la pantalla principal.
    if not cursos:
        ...
        return
    n = len(notas)#cantidad de notas, en la ista
    for i in range(n):
